...
export const modelDataCompil = (modelData) => {
  ...
  const manufacturerDataBank = listOfManufacturer;
  for (let i = 0; i < manufacturerDataBank.length; i++) {
    if (manufacturerDataBank[i].name === modelData.manufacturer) {
      manufacturerLogo = manufacturerDataBank[i].logo;
      break;
    }
  }
  ...
  const reponse = {
    ...
    constructorChpt: {
      position: constructorChpt.position,
      points: constructorChpt.points,
    },
